...
/* Main mpfit library routines (double precision) 
   $Id: mpfit.cc,v 1.1.1.1 2011/09/16 10:16:42 jaime Exp $
 */
	  ...
	  mp_result *result)
{
  ...
  double *step = 0, *dstep = 0, *llim = 0, *ulim = 0, *maxchange = 0;
  int *pfixed = 0, *mpside = 0, *ifree = 0, *qllim = 0, *qulim = 0, *do_maxchange = 0;
  int *ddebug = 0;
  ...
  if (pars) {
    ...
    mp_malloc(maxchange, double, nfree);
    mp_malloc(do_maxchange, int, nfree);
    ...
    for (i=0; i<nfree; i++) {
      ...
      if(pars[ifree[i]].do_maxchange > 0){
	do_maxchange[i] = pars[ifree[i]].do_maxchange;
	maxchange[i] = fabs(pars[ifree[i]].maxchange);
      }else{
	maxchange[i] = +0.0;
	do_maxchange[i] = 0;
      }
 ...
 L200:
  ...
  if (qanylim == 0) {
    ...
    for (j=0; j<nfree; j++) {
      ...
      /* --- jdlcr: Do not allow for changes larger than especified 
	 in params struct --- */
      
      if(do_maxchange[j] > 0)
	if(fabs(wa1[j]) > maxchange[j])
	  wa1[j] *= maxchange[j] / fabs(wa1[j]);
 ...
 CLEANUP:
  ...
  if (llim)  free(llim);
  if (ulim)  free(ulim);
  if (maxchange) free(maxchange);
  if (do_maxchange) free(do_maxchange);
