...
int set_nodes(nodes_t &n, vector<double> &itau, bool verbose){
  int nn = n.nnodes;
  string inam = "set_nodes: ";
...
void equidist(vector<double> &var, vector<double> &itau){
  int n = (int)var.size();
  ...
  if(n == 1) var[0] = 0.0;
    ...
    for(int ii = 1; ii<n-1; ii++){
      double xloc = (mma-mmi) * ii / (n-1.0) + mmi;
